...
_________________
"""
api = FastAPI(title='StupidOCR For GUI 开发者文档', version="2.0.1", description=description)

@api.post("/identify_GeneralCAPTCHA", summary='识别图片内文字', description='普通图片验证码识别，上传图片的Base64编码',
          tags=['图片验证码识别'])
async def identify_GeneralCAPTCHA(request: Request,
    ...
    return {"result": res,
            "BiliBili": "https://space.bilibili.com/37887820", "supports": "http://" + ip + ":"+ui.self.text_Port.toPlainText()+"/support"}
...
async def identify_ArithmeticCAPTCHA(request: Request,
    ...
    try:
        ...
        return {
            ...
            "supports": "http://" + ip + ":" + ui.self.text_Port.toPlainText() + "/support"
        }
...
@api.post("/SliderMode_AloneGap", summary='缺口为滑动的单独图片，返回坐标', description='识别模型1：缺口图片为单独图片',
          tags=['滑块验证码识别'])
...
@api.post("/SliderMode_Comparison", summary='缺口原图和完整原图识别，无单独滑动的缺口图片，返回坐标', description='识别模型2：一张为有缺口原图，一张为完整原图',
          tags=['滑块验证码识别'])
async def SliderMode_Comparison(request: Request,HaveGap_ImageBase64: str = Body(..., title='拥有缺口图片的Bse64文本', embed=True),
...
async def ClickChoice_CAPTCHA(request: Request,
                              ClickChoice_ImageBase64: str = Body(..., title='点选图片的Base64编码', embed=True)):
    client_ip = request.client.host
    current_time = datetime.datetime.now().strftime("%H:%M:%S")
    log_text = f"{current_time} : {client_ip} Accessing - WebApi - /ClickChoice_Txt_CAPTCHA"
    ui.text_Log.appendPlainText(log_text)
...
@api.post("/ClickChoice_Ico_CAPTCHA", summary='图标点选验证码，返回坐标', description='点选识别返回坐标',
          tags=['点选类验证码'])
async def ClickChoice_CAPTCHA(request: Request,
                              ClickChoiceCAPTCHA_Ico: str = Body(..., title='ico图标的的Base64编码', embed=True),
                              Ico_ClickChoiceCAPTCHA: str = Body(..., title='点选验证码图片Base64编码', embed=True)):
    client_ip = request.client.host
    current_time = datetime.datetime.now().strftime("%H:%M:%S")
...
@api.post("/api.IconReconnaissance", summary='Icon点选-opencv编写待优化',
          description='上传icon图标和背景图标，返回每个目标的xywh和中心点，准确率随缘，等待更新', tags=['Icon侦察'])
async def IcoReconnaissance(request: Request, Icon_ImageBase64: str = Body(..., title='Icon图标Base64', embed=True),
